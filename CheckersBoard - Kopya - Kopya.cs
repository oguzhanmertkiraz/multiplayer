...
public class CheckersBoard : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (isWhiteTurn)
        {
            txtTurn.text = "Beyazın Sırası";
        }
        else
        {
            txtTurn.text = "Siyahın Sırası";
        }
        foreach (Transform t in highlightsContainer.transform)
        {
    ...
    public void EndMovelessTurn()
    {
        if (client.isHost)
        {
            //string msg = "CEMOV";
            Server.instance.Broadcast("SEMOV", Server.instance.clients);
            //client.Send(msg);
        }
    ...
    private void Victory(bool isWhite)
    {
        ...
        if (isWhite)
            Alert("Beyaz oyuncu kazandı!");
        else
            Alert("Siyah oyuncu kazandı!");
    ...
    public void SendScore(string _score1, string _score2)
    {
        client.Send("CSCORE|" + _score1 + "|" + _score2);
    }

    public void SendTime(string time)
    {
        client.Send("CTIME|" + time);
    }

    public void writeScore(int score1, int score2)
    {
        txtScore1.text = score1.ToString();
        txtScore2.text = score2.ToString();
        SendScore(score1.ToString(), score2.ToString());
    }


    public void writeTime(int time)
    {
        txtTimer.text = time.ToString() + " saniye";
        SendTime(time.ToString());
    }


    public void writeScoreLocaly(string scoreValue1, string scoreValue2)
    {
        txtScore1.text = scoreValue1;
        txtScore2.text = scoreValue2;

    }

    public void writeTimeLocaly(string _time)
    {
        txtTimer.text = _time + " saniye";
